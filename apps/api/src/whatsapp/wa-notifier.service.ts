...
export class WaNotifierService {
  ...
  async sendTextWithButtons(to: string, text: string, buttons: { id: string; title: string }[]): Promise<void> {
    if (!this.token || !this.phoneNumberId) {
      this.logger.warn(`[DEV] WA buttons → ${to}: ${text} | buttons: ${buttons.map(b => b.title).join(', ')}`);
      return;
    }
    try {
      await fetch(this.apiUrl, {
